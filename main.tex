...
\subsubsection{Action profile prediction, strategy evaluation, and search}
...
current state $x_t$ for evaluation. In order to do so, an agent needs to 
...
%\vspace{-.2em}
\subsubsection{Online solutions using imperfect models}
%\vspace{-.2em}
...
Therefore we are proposing the solution of using an \textit{online game 
strategies}; instead of running search algorithms to solve for the total 
...
current time $t$, for as much lookahead horizon as computational resource 
...
With either form of observations $o_{t:t+\tau} \in O$, where $1/\tau$ is the 
update rate and $O$ is the observation space, agents can better infer about 
future motions of the other agents, either through the low-level motion model 
$u^{-i}_t \sim g^{-i}(x_t,\theta^{-i}_t)$ or high-level action profile model
for planning in the high-level 
action space $A^i$ on Markov 
games with time-variant types, the agent needs to update his beliefs of the following:

(a) Type estimate of other agents $b^i_t(\theta_t^{-i})$: 
$p(\theta^{-i}_t|o_{0:t})$, in order to sample the state transitions function $\mathcal{T}$ and to compute reward $r^i_t$.
