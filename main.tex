...
(a) Future strategy profiles of other agents $b^i_t(\sigma^{-i}_{t:T|I^i_l})$: 
the strategy profile prediction based on history plays, $p(\sigma_h|I^i_l)$, 
...
anticipation of other agents' $p(\sigma_{l:L'}|I^i_l)$. This online re-planning 
...
observations, it is assumed to be not directly observable throughout the 
interaction based on domain-related parameter complexity. It can be treated as 
the \textit{mental state} on human agents, which are 
hidden from other agents, sometimes, even from themselves.  

Given prior estimate of such hidden parameter, 
...
online with finite-step lookahead, $L^H<T$. Finite-step lookahead can be either 
